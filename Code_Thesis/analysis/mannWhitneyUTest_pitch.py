...
for s in sample_id:
    for f in feature:
        for g in gender:
            _df = df.loc[(df['sample_id'] == s) & (df['voice_gender'] == g)]

            # data
            group1_data = _df.loc[df['google_pitch'] == pitch[0]][f]
            group2_data = _df.loc[df['google_pitch'] == pitch[1]][f]
...
df_mWUT = pd.DataFrame({'sample': sample_list, 'feature': feature_list, 'gender': gender_list, 'p_value': p_value_list})    #, 'mWUT_result': test_result_list})
